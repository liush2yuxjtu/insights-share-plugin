...
Bundle-local runtime。给已安装 plugin 的 SessionStart wrapper 直接调用。
仅 stdlib。
"""
    ...
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT_SECONDS) as resp:
            ...
            if status == 304:
                ...
                sys.stderr.write("[session_start_full_fetch] 304 Not Modified, cache stays valid\n")
                return 0
            payload = json.loads(resp.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
        if exc.code == 304:
            sys.stderr.write("[session_start_full_fetch] 304 from daemon, cache stays valid\n")
            return 0
        sys.stderr.write(f"[session_start_full_fetch] HTTP {exc.code} fetching {url}: {exc}\n")
        return 0
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        sys.stderr.write(f"[session_start_full_fetch] network error {url}: {exc}\n")
        return 0
    except json.JSONDecodeError as exc:
        sys.stderr.write(f"[session_start_full_fetch] JSON parse error: {exc}\n")
        return 0
